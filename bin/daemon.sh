...
source /root/vos/bin/service_config.sh
...
echo "ser: $ser" 
arr=(${ser//;/ })
serviceNameLo=${arr[1]}                                  # service name with the first letter in lowercase
serviceName=${arr[0]}                                  # service name
serviceClass=${arr[2]}  
serviceUser="root"                                         # OS user name for the service
serviceGroup="root"                                    # OS group name for the service
applDir="/root/vos/bin"                          # home directory of the service application
serviceUserHome="/$serviceUser"                       # home directory of the service user
serviceLogFile="/root/vos/logs/$serviceNameLo.log"               # log file for StdOut/StdErr
maxShutdownTime=15                                         # maximum number of seconds to wait for the daemon to terminate normally
pidFile="/root/vos/bin/$serviceNameLo.pid"                      # name of PID file (PID = process ID number)
javaCommand="java"                                         # name of the Java launcher without the path
javaExe="$JAVA_HOME/bin/$javaCommand"                      # file name of the Java application launcher executable
javaArgs="-Dil.co.vor.ServiceName="$name" -cp /root/vos/bin/lib/*:/root/vos/bin/classes/ $serviceClass"                     # arguments for Java launcher
javaCommandLine="$javaExe $javaArgs"                       # command line to start the Java service application
javaCommandLineKeyword="$serviceClass"                     # a keyword that occurs on the commandline, used to detect an already running service process and to distinguish it from others
