...
public class GetUploadFiles extends Observable implements Runnable {

	private int _numberOfFTPFiles;
	private Logger _logger = Logger.getLogger(GetUploadFiles.class.getName());
	...
	public void run() {
		try {
			...
			_logger.log(Level.INFO, "New " + _numberOfFTPFiles + " Upload files");
			_ftpFiles = ServiceParameters.getDalDataClient().getFtpFiles().getFtpFilesForUpload(_serviceID, _numberOfFTPFiles).getApiData().get("ftp_files");
			...
			_logger.log(Level.SEVERE, "GetUploadFiles run failed " + e.getMessage());
