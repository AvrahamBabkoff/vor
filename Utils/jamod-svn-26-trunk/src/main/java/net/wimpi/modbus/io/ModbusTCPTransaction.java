...
public class ModbusTCPTransaction
	...
	private static Logger _logger = Logger.getLogger(ModbusTCPTransaction.class.getName());
  ...
  public void execute() throws ModbusIOException,
    ...
    try {
      ...
      while (retryCounter <= m_Retries) {
        ...
        {
      	  _logger.log(Level.SEVERE, "in ModbusIOException");

          if (retryCounter == m_Retries) {
        	  _logger.log(Level.SEVERE, "in ModbusIOException. calling close");
        	  m_Connection.close();
            throw new ModbusIOException("Executing transaction failed (tried " + m_Retries + " times)");
          } else {
        	  _logger.log(Level.SEVERE, "in ModbusIOException. increasing counter");
            retryCounter++;
            continue;
          }
        }
